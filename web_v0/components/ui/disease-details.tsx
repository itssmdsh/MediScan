...
import { Info, AlertTriangle, Stethoscope, ShieldCheck } from "lucide-react"
...
export function DiseaseDetails({ disease }: DiseaseDetailsProps) {
  ...
  const diseaseInfo: Record<string, { description: string; causes: string; symptoms: string; prevention: string }> = {
    ...
    Unknown_Normal: {
      description:
        "Your skin appears to be healthy or the condition is not one of the specific diseases this system is trained to detect.",
      causes:
        "Healthy skin is maintained through a combination of genetic factors, proper skincare, nutrition, hydration, and protection from environmental damage.",
      symptoms:
        "Healthy skin typically has a uniform color, smooth texture, and no unusual growths, rashes, or discoloration.",
      prevention:
        "Maintain healthy skin by using sunscreen daily, staying hydrated, eating a balanced diet rich in antioxidants, cleansing gently, moisturizing regularly, and avoiding smoking and excessive alcohol consumption.",
    },
  }
  ...
  const tabItems = [
    ...
    { id: "causes", label: "Causes", icon: <AlertTriangle className="h-4 w-4" /> },
    { id: "symptoms", label: "Symptoms", icon: <Stethoscope className="h-4 w-4" /> },
    ...
    <Card className="overflow-hidden border-0 shadow-lg glass-card">
      <CardHeader className="pb-2">
        <CardTitle className="text-xl">
          {disease === "Unknown_Normal" ? "Healthy Skin" : `${disease} Information`}
        </CardTitle>
      </CardHeader>
      <CardContent>
          ...
          {tabItems.map((item, index) => (
            <TabsContent key={item.id} value={item.id} className="pt-4">
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                <p className="text-sm leading-relaxed">{info[item.id as keyof typeof info]}</p>
              </motion.div>
            </TabsContent>
          ))}
        </Tabs>
